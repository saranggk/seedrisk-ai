...
## `data/wimbledon_backtest_matches.json` — real match data (separate file)

Unlike `sample_matches.json` above, `data/wimbledon_backtest_matches.json` contains **real** Wimbledon ATP/WTA match data (2021-2025), generated by `backend/scripts/wimbledon_backtest/` for offline backtesting — it does not power the live dashboard. See `docs/plans/2026-07-20-001-feat-real-wimbledon-backtest-data-plan.md` for its shape and how it's derived.

**Attribution:** the underlying match data was compiled by Jeff Sackmann and sourced via [`Aneeshers/tennis-sackmann-archive`](https://github.com/Aneeshers/tennis-sackmann-archive), an archival mirror of Jeff Sackmann's `tennis_atp`/`tennis_wta` datasets. Both the original data and this mirror are licensed under [Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International (CC BY-NC-SA 4.0)](https://creativecommons.org/licenses/by-nc-sa/4.0/).

## `data/calibration_report.json` — model calibration diagnostic (separate file)

Generated by `backend/scripts/backtest_calibration/` from `data/wimbledon_backtest_matches.json`: runs the live rule-based `predict_upset()` model against every scoreable match and reports how well its predicted probabilities and risk labels track the real outcomes (overall Brier score, a reliability diagram, and breakdowns by tour and by risk label). Diagnostic only — it does not feed back into the live app or change `upset_model.py`. See `docs/plans/2026-07-21-001-feat-backtest-calibration-harness-plan.md`.
